...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $data_vencimento = converterDataBrasil(trim($_POST['data_vencimento'] ?? '')); // NOVO CAMPO
        ...
        try {
            $check_columns = $pdo->query("SHOW COLUMNS FROM compras LIKE 'data_%'");
        ...
        if ($columns_exist) {
            $sql = "INSERT INTO compras 
                    (fornecedor, numero_nf, produto, quantidade, valor_unitario, valor_total, frete, link_pagamento, numero_empenho, observacao, data, data_pagamento_compra, data_pagamento_frete, data_vencimento, comprovante_pagamento) 
                    VALUES 
                    (:fornecedor, :numero_nf, :produto, :quantidade, :valor_unitario, :valor_total, :frete, :link_pagamento, :numero_empenho, :observacao, :data, :data_pagamento_compra, :data_pagamento_frete, :data_vencimento, :comprovante_pagamento)";
        } else {
            $sql = "INSERT INTO compras 
        ...
        if ($columns_exist) {
            ...
            $stmt->bindParam(':data_vencimento', $data_vencimento, PDO::PARAM_STR); // NOVO BIND
        }
        ...
        logUserAction('CREATE', 'compras', $compra_id, [
            ...
            'data_vencimento' => $data_vencimento, // NOVO CAMPO NO LOG
        ...
        <div class="data-section">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <small style="color: var(--medium-gray); font-size: 0.85rem; margin-top: 0.25rem; display: block;">
                        <i class="fas fa-info-circle"></i> Data limite para pagamento da compra
                    </small>
                </div>
            </div>
        </div>
...
function validarFormulario() {
    ...
    const dataVencimento = document.getElementById("data_vencimento"); // NOVO CAMPO
    ...
    // NOVA VALIDAÇÃO - Data de vencimento (opcional, mas se preenchida deve ser válida)
    ...
    // NOVA VALIDAÇÃO - Validar se a data de vencimento não é anterior à data da compra
    if (data.value && dataVencimento && dataVencimento.value) {
        const dataCompra = new Date(data.value);
...
function limparFormulario() {
    ...
    if (dataVencimento) dataVencimento.value = ''; // LIMPAR NOVO CAMPO
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // NOVO EVENT LISTENER para data de vencimento
    if (dataVencimento) {
...
console.log('🚀 Sistema de Cadastro de Compras LicitaSis v4.0:', {
    versao: '4.0 Data de Vencimento',
    novosCampos: [
        '✅ Data de Pagamento da Compra',
        '✅ Data de Pagamento do Frete',
    ...
    estrutura_bd: {
        compras: 'Tabela principal com dados básicos + datas de pagamento + data vencimento',
    ...
    validacoes: [
        ...
        '✅ Validação de data de vencimento >= data da compra',
