...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status']) && $permissionManager->hasPagePermission('financeiro', 'edit')) {
    ...
    $data_recebimento = $_POST['data_recebimento'] ?? null; // NOVO
    ...
    if ($id > 0 && in_array($novo_status, ['Não Recebido', 'Recebido'])) {
        ...
        if ($novo_status === 'Recebido') {
            ...
            // NOVO: Validar data de recebimento para status "Recebido"
            if (empty($data_recebimento)) {
                echo json_encode(['success' => false, 'error' => 'Data de recebimento é obrigatória']);
                exit();
            }
            
            // NOVO: Validar formato da data
            $data_recebimento = converterDataParaMySQL($data_recebimento);
            if (!$data_recebimento) {
                echo json_encode(['success' => false, 'error' => 'Data de recebimento inválida']);
                exit();
            }
        } else {
            // Se está alterando para "Não Recebido", limpa a data de recebimento
        ...
        try {
            ...
            // NOVO: SQL modificado para incluir data_recebimento
            $sql = "UPDATE vendas SET status_pagamento = :status, data_recebimento = :data_recebimento WHERE id = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->bindValue(':status', $novo_status);
            $stmt->bindValue(':data_recebimento', $data_recebimento);
            $stmt->bindValue(':id', $id, PDO::PARAM_INT);
            $stmt->execute();            
            // Busca dados novos para auditoria
            $newDataStmt = $pdo->prepare("SELECT * FROM vendas WHERE id = :id");
...
<div id="confirmationModal" class="confirmation-modal" role="dialog" aria-modal="true">
    <div class="confirmation-modal-content">
        ...
        <!-- NOVO: Campo de Data de Recebimento -->
        <div class="data-recebimento-section" style="margin: 1.5rem 0; padding: 1rem; background: #e7f3ff; border-radius: var(--radius); border-left: 4px solid var(--info-color);">
            <h4 style="color: var(--info-color); margin-bottom: 1rem; font-size: 1rem;">
                <i class="fas fa-calendar-check"></i> Data de Recebimento
            </h4>
            <div class="form-group">
                <label for="dataRecebimento" style="font-weight: 600; margin-bottom: 0.5rem; display: block;">
                ...
                <input type="date" 
                       ...
                       class="form-control" 
                       required
                       style="width: 100%; padding: 0.75rem; border: 2px solid var(--border-color); border-radius: var(--radius);">
                <small style="color: var(--medium-gray); font-size: 0.85rem; margin-top: 0.25rem; display: block;">
                    Informe a data real em que o pagamento foi recebido
                </small>
            </div>
        </div>
...
window.requestFinancialAuth = function() {
    ...
    if (!dataRecebimento) {
        showToast('Por favor, informe a data de recebimento', 'error');
        document.getElementById('dataRecebimento').focus();
        return;
    }
    ...
    const hoje = new Date();
    const dataInformada = new Date(dataRecebimento);
    
    if (dataInformada > hoje) {
        const confirmar = confirm(
            'A data de recebimento está no futuro.\n\n' +
            'Tem certeza que deseja continuar com esta data?\n\n' +
            'Data informada: ' + dataInformada.toLocaleDateString('pt-BR')
        );
    ...
    // Salvar a data no objeto de dados da conta
    currentContaData.dataRecebimento = dataRecebimento;
