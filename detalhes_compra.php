...
if (file_exists('includes/audit.php')) {
    ...
    logUserAction('READ', 'detalhes_compra');
}
...
if ($compra_id <= 0) {
    $error = "ID da compra não informado ou inválido.";
} else {
    try {
        // Primeiro, vamos verificar quais colunas existem na tabela fornecedores
        $columns_query = "SHOW COLUMNS FROM fornecedores";
        ...
        $stmt->execute();
        $compra_info = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        if (!$compra_info) {
            ...
            $sql_produtos = "SELECT 
                                ...
                                p.und,
                                p.categoria,
                                p.fornecedor as produto_fornecedor_id,
                                ...
                                p.custo_total,
                                p.preco_venda,
                                pf.nome as produto_fornecedor_nome
            ...
            $stmt->bindParam(':compra_id', $compra_id, PDO::PARAM_INT);
            $stmt->execute();
            $produtos_compra = $stmt->fetchAll(PDO::FETCH_ASSOC);
            
            // Calcula totais
            foreach ($produtos_compra as $produto) {
                $totais['quantidade_total'] += intval($produto['quantidade'] ?? 0);
                $totais['valor_total'] += floatval($produto['valor_total'] ?? 0);
            }
            $totais['frete_total'] = floatval($compra_info['frete'] ?? 0);
            $totais['valor_com_frete'] = $totais['valor_total'] + $totais['frete_total'];
            
            // Busca informações do fornecedor se disponível
            if (!empty($compra_info['fornecedor'])) {
                // Tenta buscar por nome primeiro
                $sql_fornecedor = "SELECT * FROM fornecedores WHERE nome = :fornecedor_nome";
                $stmt = $pdo->prepare($sql_fornecedor);
                $stmt->bindParam(':fornecedor_nome', $compra_info['fornecedor'], PDO::PARAM_STR);
                $stmt->execute();
                $fornecedor_info = $stmt->fetch(PDO::FETCH_ASSOC);
                
                // Se não encontrar por nome, tenta buscar por ID
                if (!$fornecedor_info && is_numeric($compra_info['fornecedor'])) {
                    $sql_fornecedor = "SELECT * FROM fornecedores WHERE id = :fornecedor_id";
                    $stmt = $pdo->prepare($sql_fornecedor);
                    $stmt->bindParam(':fornecedor_id', $compra_info['fornecedor'], PDO::PARAM_INT);
                    $stmt->execute();
                    $fornecedor_info = $stmt->fetch(PDO::FETCH_ASSOC);
                }
            }
...
if (file_exists('includes/header_template.php')) {
    ...
    renderHeader("Detalhes da Compra #$compra_id - LicitaSis", "compras");
} else {
    ...
    <head>
        ...
        <title>Detalhes da Compra #<?php echo $compra_id; ?> - LicitaSis</title>
...
<div class="container">
    <div class="page-header">
        <h1><i class="fas fa-shopping-cart"></i> Detalhes da Compra #<?php echo $compra_id; ?></h1>
        <div class="compra-info">
            <?php if ($compra_info): ?>
                <i class="fas fa-calendar"></i> 
                Data: <?php echo date('d/m/Y', strtotime($compra_info['data'])); ?> | 
                <i class="fas fa-file-invoice"></i> 
                Nota Fiscal: <?php echo htmlspecialchars($compra_info['numero_nf'] ?? 'N/A'); ?>
                <?php if (!empty($compra_info['numero_empenho'])): ?>
                    | <i class="fas fa-receipt"></i> 
                    Empenho: <?php echo htmlspecialchars($compra_info['numero_empenho']); ?>
                <?php endif; ?>
            <?php endif; ?>
        </div>
    </div>
    ...
    <?php if ($compra_info): ?>
        <!-- Grid de informações da compra e fornecedor -->
        <div class="content-grid">
            <!-- Informações da Compra -->
            <div class="info-card">
                <h3><i class="fas fa-shopping-cart"></i> Informações da Compra</h3>
                <div class="info-grid">
                    <div class="info-item">
                        <div class="info-label">ID da Compra</div>
                        <div class="info-value">#<?php echo $compra_info['id']; ?></div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Data da Compra</div>
                        <div class="info-value">
                            <?php 
                                if (!empty($compra_info['data']) && $compra_info['data'] !== '0000-00-00') {
                                    echo date('d/m/Y', strtotime($compra_info['data'])); 
                                } else {
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value">
                            <?php echo !empty($compra_info['numero_nf']) ? htmlspecialchars($compra_info['numero_nf']) : 'N/A'; ?>
                        </div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Número do Empenho</div>
                        <div class="info-value">
                            <?php echo !empty($compra_info['numero_empenho']) ? htmlspecialchars($compra_info['numero_empenho']) : 'N/A'; ?>
                        </div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Valor do Frete</div>
                        <div class="info-value currency">
                            R$ <?php echo number_format(floatval($compra_info['frete'] ?? 0), 2, ',', '.'); ?>
                        </div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Status do Pagamento</div>
                        <div class="info-value status">
                            <?php if (!empty($compra_info['comprovante_pagamento'])): ?>
                                <span class="status-badge status-pago">
                                    <i class="fas fa-check-circle"></i> Pago
                                </span>
                            <?php elseif (!empty($compra_info['link_pagamento'])): ?>
                                <span class="status-badge status-pendente">
                                    <i class="fas fa-clock"></i> Pendente
                                </span>
                            <?php else: ?>
                                <span class="status-badge status-registrado">
                                    <i class="fas fa-info-circle"></i> Registrado
                                </span>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="info-item" style="grid-column: 1 / -1;">
                        ...
                        <div class="info-value">
                            <?php echo !empty($compra_info['observacao']) ? htmlspecialchars($compra_info['observacao']) : 'Nenhuma observação'; ?>
                        </div>
                    </div>
                    <div class="info-item">
                        ...
                        <div class="info-value">
                            <?php 
                                if (!empty($compra_info['created_at'])) {
                                    echo date('d/m/Y H:i', strtotime($compra_info['created_at'])); 
                                } else {
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value">
                            <?php 
                                if (!empty($compra_info['updated_at'])) {
                                    echo date('d/m/Y H:i', strtotime($compra_info['updated_at'])); 
                                } else {
            ...
            <div class="info-card">
                <h3><i class="fas fa-industry"></i> Informações do Fornecedor</h3>
                <?php if ($fornecedor_info): ?>
                    <div class="info-grid">
                        <div class="info-item">
                            <div class="info-label">Nome da Empresa</div>
                            <div class="info-value">
                                <?php echo htmlspecialchars($fornecedor_info['nome'] ?? 'Não informado'); ?>
                            </div>
                        </div>
                        <div class="info-item">
                            <div class="info-label">Código</div>
                            <div class="info-value">
                                <?php echo htmlspecialchars($fornecedor_info['codigo'] ?? 'N/A'); ?>
                            </div>
                        </div>
                        <div class="info-item">
                            ...
                            <div class="info-value">
                                <?php 
                                    if (!empty($fornecedor_info['cnpj'])) {
                                        // Formata CNPJ se necessário
                                        $cnpj = preg_replace('/\D/', '', $fornecedor_info['cnpj']);
                                        if (strlen($cnpj) === 14) {
                                            ...
                                            echo htmlspecialchars($fornecedor_info['cnpj']);
                                        }
                                    } else {
                                        echo 'Não informado';
                                    }
                        ...
                        <div class="info-item">
                            ...
                            <div class="info-value">
                                <?php echo htmlspecialchars($fornecedor_info['responsavel'] ?? 'Não informado'); ?>
                            </div>
                        </div>
                        <div class="info-item">
                            <div class="info-label">Telefone</div>
                            <div class="info-value">
                                <?php echo htmlspecialchars($fornecedor_info['telefone'] ?? 'Não informado'); ?>
                            </div>
                        </div>
                        <div class="info-item">
                            <div class="info-label">E-mail</div>
                            <div class="info-value">
                                <?php echo htmlspecialchars($fornecedor_info['email'] ?? 'Não informado'); ?>
                            </div>
                        </div>
                        <div class="info-item" style="grid-column: 1 / -1;">
                            ...
                            <div class="info-value">
                                <?php 
                                    $endereco_completo = '';
                                    if (!empty($fornecedor_info['endereco'])) $endereco_completo .= $fornecedor_info['endereco'];
                                    if (!empty($fornecedor_info['cidade'])) $endereco_completo .= (!empty($endereco_completo) ? ', ' : '') . $fornecedor_info['cidade'];
                                    if (!empty($fornecedor_info['estado'])) $endereco_completo .= (!empty($endereco_completo) ? ' - ' : '') . $fornecedor_info['estado'];
                                    if (!empty($fornecedor_info['cep'])) $endereco_completo .= (!empty($endereco_completo) ? ' - CEP: ' : 'CEP: ') . $fornecedor_info['cep'];
                ...
                <?php else: ?>
                    <div class="info-item">
                        ...
                        <div class="info-value">
                            <?php echo htmlspecialchars($compra_info['fornecedor'] ?? 'Não informado'); ?>
                        </div>
                    </div>
                    <p style="color: var(--medium-gray); text-align: center; margin-top: 1rem;">
                        <i class="fas fa-info-circle"></i> Informações detalhadas do fornecedor não disponíveis
                    </p>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="produtos-card">
            <h3><i class="fas fa-boxes"></i> Produtos da Compra</h3>
            <?php if (count($produtos_compra) > 0): ?>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th><i class="fas fa-industry"></i> Fornecedor do Produto</th>
                        ...
                        <tbody>
                            <?php foreach ($produtos_compra as $produto): ?>
                                <tr>
                                    <td>
                                    ...
                                    <td>
                                        ...
                                        <?php if ($produto['produto_id']): ?>
                                            <br><small style="color: var(--medium-gray);">ID: <?php echo $produto['produto_id']; ?></small>
                                        <?php endif; ?>
                                    </td>
                                    <td class="center">
                                    ...
                                    <td>
                                        <?php echo !empty($produto['produto_fornecedor_nome']) ? htmlspecialchars($produto['produto_fornecedor_nome']) : 'Não informado'; ?>
                                    </td>
                                    <td class="center">
        ...
        <div class="totais-card">
            ...
            <div class="totais-grid">
                ...
                <div class="total-item">
                    ...
                    <div class="total-value">R$ <?php echo number_format($totais['frete_total'], 2, ',', '.'); ?></div>
                </div>
                <div class="total-item">
                    <div class="total-label">Total da Compra</div>
                    <div class="total-value">R$ <?php echo number_format($totais['valor_com_frete'], 2, ',', '.'); ?></div>
                </div>
            </div>
        </div>

        <!-- Links e Documentos -->
        <?php if (!empty($compra_info['link_pagamento']) || !empty($compra_info['comprovante_pagamento'])): ?>
        <div class="info-card" style="margin-bottom: 2rem;">
            <h3><i class="fas fa-link"></i> Links e Documentos</h3>
            <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                <?php if (!empty($compra_info['link_pagamento'])): ?>
                    <a href="<?php echo htmlspecialchars($compra_info['link_pagamento']); ?>" 
                       target="_blank" 
                       class="btn btn-info">
                ...
                <?php if (!empty($compra_info['comprovante_pagamento'])): ?>
                    <a href="<?php echo htmlspecialchars($compra_info['comprovante_pagamento']); ?>" 
                       target="_blank" 
    ...
    <div class="btn-container">
        <a href="consulta_compras.php" class="btn btn-secondary">
        ...
        <?php if ($compra_info): ?>
            ...
            <a href="exportar_compra.php?id=<?php echo $compra_id; ?>" class="btn btn-success">
                <i class="fas fa-file-excel"></i> Exportar Excel
            </a>
            
            <?php if ($isAdmin): ?>
                <a href="editar_compra.php?id=<?php echo $compra_id; ?>" class="btn btn-warning">
                    <i class="fas fa-edit"></i> Editar Compra
                </a>
            <?php endif; ?>
            
            <a href="gerar_pdf_compra.php?id=<?php echo $compra_id; ?>" class="btn btn-primary" target="_blank">
            ...
            <?php if (!empty($compra_info['link_pagamento'])): ?>
                <a href="<?php echo htmlspecialchars($compra_info['link_pagamento']); ?>" 
                   target="_blank" 
                   class="btn btn-info">
                    <i class="fas fa-credit-card"></i> Acessar Pagamento
                </a>
            <?php endif; ?>
        <?php endif; ?>
    </div>
...
<script>
    // JavaScript específico da página de detalhes da compra
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const exportButton = document.querySelector('a[href*="exportar_compra"]');
        if (exportButton) {
            exportButton.addEventListener('click', function(e) {
                if (!confirm('Deseja exportar os dados desta compra para Excel?')) {
                    e.preventDefault();
                }
            });
        }

        const editButton = document.querySelector('a[href*="editar_compra"]');
        if (editButton) {
            editButton.addEventListener('click', function(e) {
                if (!confirm('Deseja editar esta compra? Esta ação pode afetar relatórios e controles financeiros.')) {
                    e.preventDefault();
                }
            });
        }
        ...
        document.addEventListener('keydown', function(e) {
            ...
            if ((e.ctrlKey || e.metaKey) && e.key === 'b') {
                e.preventDefault();
                window.location.href = 'consulta_compras.php';
            }
        ...
        console.log('Página de Detalhes da Compra carregada com sucesso!');
        console.log('Compra ID:', <?php echo $compra_id; ?>);
        console.log('Total de produtos:', <?php echo count($produtos_compra); ?>);
        console.log('Valor total da compra: R$ <?php echo number_format($totais['valor_com_frete'], 2, ',', '.'); ?>');
        ...
        window.addEventListener('beforeunload', function() {
            localStorage.setItem('detalhes_compra_scroll', window.scrollY);
        });

        // Restaura posição do scroll
        const savedScroll = localStorage.getItem('detalhes_compra_scroll');
        if (savedScroll) {
            window.scrollTo(0, parseInt(savedScroll));
            localStorage.removeItem('detalhes_compra_scroll');
        }
    });
