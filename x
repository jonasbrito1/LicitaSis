...
/*
===========================================================================
SISTEMA DE CONTAS A PAGAR - LICITASIS v7.3
===========================================================================
...
- ✅ NOVO: Sistema completo de filtros de busca
- ✅ NOVO: Filtro por status com estatísticas em tempo real
- ✅ NOVO: Contadores por status nos filtros
- ✅ NOVO: Atalhos de teclado para navegação rápida
...
1. Filtro de texto: Busca por NF, fornecedor ou status
2. Filtro por status: Pendente, Pago, Concluído
3. Combinação de filtros: Texto + Status
4. Estatísticas em tempo real: Contadores por status
5. Auto-submit: Mudanças aplicadas automaticamente
6. Indicadores visuais: Cores por status, badges informativos
7. Botão "Limpar": Remove todos os filtros rapidamente

ATALHOS DE TECLADO:
- Ctrl+F: Focar no campo de busca
- Ctrl+S: Focar no filtro de status  
- Escape: Limpar filtros (quando campo focado)
...
function buscarContas($pdo, $searchTerm = '', $statusFilter = '') {
    $contas = [];
    ...
    try {
        ...
        if (!empty($searchTerm)) {
            $conditions[] = "(c.numero_nf LIKE :searchTerm 
        ...
        if (!empty($statusFilter) && $statusFilter !== 'todos') {
            // Mapeia o status para garantir consistência
            $statusMap = [
                'Pendente' => 'Pendente',
                'pendente' => 'Pendente',
                'Pago' => 'Pago',
            ...
            $mappedStatus = $statusMap[$statusFilter] ?? $statusFilter;
            $conditions[] = "cp.status_pagamento = :statusFilter";
            $params['statusFilter'] = $mappedStatus;
        }
        ...
        $sql .= " ORDER BY c.data DESC, cp.status_pagamento ASC";
        ...
        $logMessage = "Busca realizada";
        if (!empty($searchTerm)) {
            $logMessage .= " com termo: '$searchTerm'";
        }
        if (!empty($statusFilter) && $statusFilter !== 'todos') {
            $logMessage .= " filtrado por status: '$statusFilter'";
        }
        $logMessage .= ", resultados: " . count($contas);
        error_log("LicitaSis - $logMessage");
...
// Função para calcular estatísticas dos filtros
function calcularEstatisticasFiltros($pdo, $searchTerm = '', $statusFilter = '') {
    $stats = [
        'total_filtrado' => 0,
        'total_geral' => 0,
        'por_status' => [
    ...
    try {
        ...
        $sqlTotal = "SELECT COUNT(*) as total FROM contas_pagar cp INNER JOIN compras c ON cp.compra_id = c.id";
        $stmt = $pdo->prepare($sqlTotal);
        $stmt->execute();
        $stats['total_geral'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
        
        // Total filtrado (com os filtros aplicados)
        $sql = "SELECT COUNT(*) as total FROM contas_pagar cp INNER JOIN compras c ON cp.compra_id = c.id";
        $conditions = [];
        $params = [];
        
        if (!empty($searchTerm)) {
            $conditions[] = "(c.numero_nf LIKE :searchTerm OR c.fornecedor LIKE :searchTerm OR cp.status_pagamento LIKE :searchTerm)";
            $params['searchTerm'] = "%$searchTerm%";
        }
        ...
        $stmt->execute();
        $stats['total_filtrado'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
        
        // Contagem por status (respeitando outros filtros, mas não o filtro de status)
        foreach (['Pendente', 'Pago', 'Concluido'] as $status) {
            $sqlStatus = "SELECT COUNT(*) as total FROM contas_pagar cp INNER JOIN compras c ON cp.compra_id = c.id WHERE cp.status_pagamento = :status";
            $statusConditions = [];
            $statusParams = ['status' => $status];
            
            if (!empty($searchTerm)) {
                $statusConditions[] = "(c.numero_nf LIKE :searchTerm OR c.fornecedor LIKE :searchTerm OR cp.status_pagamento LIKE :searchTerm)";
                $statusParams['searchTerm'] = "%$searchTerm%";
            }
            
            if (!empty($statusConditions)) {
                $sqlStatus .= " AND " . implode(" AND ", $statusConditions);
            }
            ...
            $stmt->execute();
            $stats['por_status'][$status] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
        }
        
    } catch (PDOException $e) {
        error_log("LicitaSis - Erro ao calcular estatísticas: " . $e->getMessage());
    }
...
$estatisticas = calcularEstatisticasFiltros($pdo, $searchTerm, $statusFilter);

// Busca as contas com filtros
$searchTerm = isset($_GET['search']) && !empty($_GET['search']) ? $_GET['search'] : '';
$statusFilter = isset($_GET['status_filter']) && !empty($_GET['status_filter']) ? $_GET['status_filter'] : '';
$contas = buscarContas($pdo, $searchTerm, $statusFilter);
            ...
            <div class="search-bar">
                ...
                <div class="search-group">
                    ...
                    <select name="status_filter" 
                            ...
                            title="Selecione um status para filtrar as contas">
                        <option value="">🔍 Todos os Status</option>
                        <option value="Pendente" <?php echo ($statusFilter === 'Pendente') ? 'selected' : ''; ?>>
                            📅 Pendente <?php echo isset($estatisticas['por_status']['Pendente']) ? "({$estatisticas['por_status']['Pendente']})" : ''; ?>
                        </option>
                        <option value="Pago" <?php echo ($statusFilter === 'Pago') ? 'selected' : ''; ?>>
                            ✅ Pago <?php echo isset($estatisticas['por_status']['Pago']) ? "({$estatisticas['por_status']['Pago']})" : ''; ?>
                        </option>
                        <option value="Concluido" <?php echo ($statusFilter === 'Concluido') ? 'selected' : ''; ?>>
                            🎯 Concluído <?php echo isset($estatisticas['por_status']['Concluido']) ? "({$estatisticas['por_status']['Concluido']})" : ''; ?>
                        </option>
                    </select>
                </div>
                ...
                <div class="search-actions">
                    ...
                    <?php if (!empty($searchTerm) || !empty($statusFilter)): ?>
                        <a href="contas_a_pagar.php" class="btn-clear">
                ...
                <div class="search-info">
                    <div class="search-info-main">
                        ...
                        <?php if (!empty($searchTerm)): ?>
                            <span class="filter-tag">
                                <i class="fas fa-search"></i> 
                                Termo: "<?php echo htmlspecialchars($searchTerm); ?>"
                            </span>
                        <?php endif; ?>
                    ...
                    <div class="search-stats">
                        <span class="filter-results">
                            ...
                            <?php echo count($contas); ?> de <?php echo $estatisticas['total_geral']; ?> conta(s)
                        </span>
                        
                        <?php if (!empty($searchTerm) && empty($statusFilter)): ?>
                            <div class="status-breakdown">
                                <span class="status-count pendente">
                                    <i class="fas fa-clock"></i> 
                                    <?php echo $estatisticas['por_status']['Pendente']; ?> Pendente(s)
                                </span>
                                <span class="status-count pago">
                                    <i class="fas fa-check-circle"></i> 
                                    <?php echo $estatisticas['por_status']['Pago']; ?> Pago(s)
                                </span>
                                <span class="status-count concluido">
                                    <i class="fas fa-check-double"></i> 
                                    <?php echo $estatisticas['por_status']['Concluido']; ?> Concluído(s)
                                </span>
                            </div>
                        <?php endif; ?>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('   - Filtro de texto:', <?php echo !empty($searchTerm) ? '"' . addslashes($searchTerm) . '"' : 'Nenhum'; ?>);
    console.log('   - Filtro de status:', <?php echo !empty($statusFilter) ? '"' . $statusFilter . '"' : 'Todos'; ?>);
    console.log('   - Total de resultados:', <?php echo count($contas); ?>);
    console.log('   - Total geral de contas:', <?php echo $estatisticas['total_geral']; ?>);
    console.log('⌨️ ATALHOS DE TECLADO:');
    console.log('   - Ctrl+F: Focar no campo de busca');
    console.log('   - Ctrl+S: Focar no filtro de status');
    console.log('   - Escape: Limpar filtros (quando campo está focado)');
    console.log('📈 ESTATÍSTICAS POR STATUS:');
    console.log('   - Pendentes:', <?php echo $estatisticas['por_status']['Pendente']; ?>);
    console.log('   - Pagos:', <?php echo $estatisticas['por_status']['Pago']; ?>);
    console.log('   - Concluídos:', <?php echo $estatisticas['por_status']['Concluido']; ?>);
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('input', function() {
            ...
            searchTimeout = setTimeout(() => {
                ...
                if (form) {
                    console.log('🔍 Auto-submit por mudança no campo de busca');
                    form.submit();
                }
            }, 800); // Delay de 800ms
        });
    ...
    if (statusFilter) {
        statusFilter.addEventListener('change', function() {
            const form = this.closest('form');
            if (form) {
                console.log('🔍 Auto-submit por mudança no filtro de status:', this.value);
                form.submit();
            }
        });
        
        // Aplica estilo visual baseado no status selecionado
        function updateStatusFilterStyle() {
            const value = statusFilter.value;
            statusFilter.className = 'status-filter-select';
            
            if (value === 'Pendente') {
                statusFilter.style.borderColor = 'var(--pendente-color)';
                statusFilter.style.backgroundColor = 'rgba(253, 126, 20, 0.1)';
            } else if (value === 'Pago') {
                statusFilter.style.borderColor = 'var(--pago-color)';
                statusFilter.style.backgroundColor = 'rgba(40, 167, 69, 0.1)';
            } else if (value === 'Concluido') {
                statusFilter.style.borderColor = 'var(--concluido-color)';
                statusFilter.style.backgroundColor = 'rgba(23, 162, 184, 0.1)';
            } else {
                statusFilter.style.borderColor = 'var(--border-color)';
    ...
    document.addEventListener('keydown', function(e) {
        // Ctrl/Cmd + F para focar no campo de busca
        if ((e.ctrlKey || e.metaKey) && e.key === 'f') {
            ...
            if (searchInput) {
                searchInput.focus();
                searchInput.select();
                console.log('⌨️ Foco no campo de busca via atalho');
            }
        }
        
        // Ctrl/Cmd + S para focar no filtro de status
        if ((e.ctrlKey || e.metaKey) && e.key === 's') {
            e.preventDefault();
            const statusFilter = document.getElementById('status_filter');
            if (statusFilter) {
                statusFilter.focus();
                console.log('⌨️ Foco no filtro de status via atalho');
            }
        }
        ...
        if (e.key === 'Escape') {
            ...
            if (activeElement && (activeElement.id === 'search' || activeElement.id === 'status_filter')) {
                const currentUrl = window.location.pathname;
                console.log('⌨️ Limpando filtros via Escape');
                window.location.href = currentUrl;
            }
        }
    });
    
    // Adiciona indicadores visuais para atalhos
    if (searchInput) {
        searchInput.setAttribute('title', 'Digite para buscar ou use Ctrl+F para focar aqui');
        const currentPlaceholder = searchInput.getAttribute('placeholder') || '';
        if (!currentPlaceholder.includes('Ctrl+F')) {
            searchInput.setAttribute('placeholder', currentPlaceholder + ' (Ctrl+F)');
        }
    }
    
    if (statusFilter) {
        statusFilter.setAttribute('title', 'Selecione um status ou use Ctrl+S para focar aqui');
    }
